...
class MetricsTestCase(TestCase):
    def setUp(self):
        self.maxDiff = None
        settings = {"prometheus.pyramid_request.buckets": "0.001, 0.1"}
    ...
    def test_integration(self):
        ...
        metrics = {
            metric.name: metric for metric in text_string_to_metric_families(resp.text)
        }

        pyramid_request_ingress = {
            (sample.labels["method"], sample.labels["path_info_pattern"]): sample.value
        ...
        self.assertEqual(request_bucket[("GET", "/hello/{name}", "200", "0.001")], 0.0)
        self.assertEqual(request_bucket[("GET", "/hello/{name}", "200", "0.1")], 1.0)
        self.assertEqual(request_bucket[("GET", "/hello/{name}", "200", "+Inf")], 1.0)

        self.assertEqual(request_bucket[("POST", "/hello/{name}", "200", "0.001")], 0.0)
        self.assertEqual(request_bucket[("POST", "/hello/{name}", "200", "0.1")], 2.0)
        self.assertEqual(request_bucket[("POST", "/hello/{name}", "200", "+Inf")], 2.0)
        ...
        self.assertIn(("GET", "/metrics", "200", "+Inf"), request_bucket)
        ...
        self.assertEqual(
            request_count,
            {
                ("POST", "/hello/{name}", "200"): 2.0,
                ("GET", "/hello/{name}", "200"): 1.0,
                ("GET", "/metrics", "200"): 1.0,
            },
        )
