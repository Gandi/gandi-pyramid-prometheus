...
class PromTestCase(TestCase):
    def test_get_metrics(self):
        ...
        prom.pyramid_request_ingress.labels(
            path_info_pattern="/test", method="GET"
        ).set(42)

        metric = get_metrics(req)

        self.assertIn(
            "# HELP pyramid_request_ingress Number of requests currrently processed\n",
            metric.text,
        )

        self.assertIn("# TYPE pyramid_request_ingress gauge\n", metric.text)

        self.assertIn(
            'pyramid_request_ingress{method="GET",path_info_pattern="/test"} 42.0\n',
            metric.text,
        )
